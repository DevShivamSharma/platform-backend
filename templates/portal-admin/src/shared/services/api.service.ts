...
import { legacyRoutesEnabled, portalConfig, type GeneratedModule } from "@/lib/portal-config"
...
interface EntityRoute {
  ...
  readOnly?: boolean
  /** Generated module config (drives the dashboard /stats aggregation). */
  module?: GeneratedModule
}

const generatedEntityRoutes: Array<{ base: string; route: EntityRoute }> = (
  (portalConfig.generatedModules ?? []) as GeneratedModule[]
).map((module) => {
  const searchField =
    (module.listColumns || [])
      .map((key) => module.fields.find((field) => field.key === key))
      .find((field) => field && ["text", "email", "phone", "select", "radio"].includes(field.type)) ||
    module.fields.find((field) => ["text", "email", "phone", "select", "radio"].includes(field.type))

  return {
    base: `/api/v1/generated/${module.id}`,
    route: {
      table: module.tableName || module.id,
      searchColumn: searchField?.key,
...
const legacyEntityRoutes: Array<{ base: string; route: EntityRoute }> = [
...
const ENTITY_ROUTES: Array<{ base: string; route: EntityRoute }> = [
  ...generatedEntityRoutes,
  ...(legacyRoutesEnabled() ? legacyEntityRoutes : []),
]
...
export class ApiClient {
  ...
  private async route<T>(method: HttpMethod, rawUrl: string, body?: unknown, config?: RequestConfig): Promise<ApiResponse<T>> {
    ...
    if (path.endsWith("/change-password")) return this.handlePasswordChange<T>(body)
    ...
    if (match) {
      ...
      // Dashboard analytics: GET /api/v1/generated/<module>/stats
      if (rest === "stats" && method === "GET") return this.handleStats<T>(match.route)
  ...
  private async handlePasswordChange<T>(body: unknown): Promise<ApiResponse<T>> {
    assertConfigured()
    const payload = (body ?? {}) as Record<string, string>
    const { data: auth } = await supabase.auth.getUser()
    const email = auth.user?.email
    if (!email) throw new ApiClientError(401, "Session expired", "AUTH")
    ...
    if (signInError) {
      throw new ApiClientError(401, "Current password is incorrect.", "AUTH")
    }

    const { error } = await supabase.auth.updateUser({ password: payload.newPassword })
    if (error) throw new ApiClientError(400, error.message, "PASSWORD_UPDATE_FAILED")
    return ok({} as T)
  }
  ...
  private async loadProfile(authUserId: string, email: string) {
    ...
    return {
      ...
      subscriptionId: (r.subscription_id as string) || undefined,
  ...
  // ── dashboard stats (server-side aggregation via Postgres RPCs) ──
  private async handleStats<T>(route: EntityRoute): Promise<ApiResponse<T>> {
    const table = route.table
    const module = route.module
    const fields = module?.fields ?? []

    // Total row count — server-side COUNT, no rows transferred.
    const { count } = await supabase.from(table).select("*", { count: "exact", head: true })
    const total = count ?? 0

    // First categorical field drives the breakdown chart (status, class, …).
    const categoryField = fields.find((f) => ["select", "radio"].includes(f.type))
    // First numeric/currency field drives the summed metric.
    const numberField = fields.find((f) => ["currency", "number"].includes(f.type))

    let groups:
      | { field: string; label: string; data: Array<{ label: string; value: number }> }
      | undefined
    if (categoryField) {
      const { data, error } = await supabase.rpc("portal_group_count", {
        p_table: table,
        p_column: categoryField.key,
        p_limit: 8,
      })
      if (!error && Array.isArray(data)) {
        groups = {
          field: categoryField.key,
          label: categoryField.label,
          data: data.map((r: { label: unknown; value: unknown }) => ({
            label: String(r.label ?? "Unspecified"),
    ...
    let metric: { label: string; value: number; currency: boolean } | undefined
    if (numberField) {
      const { data, error } = await supabase.rpc("portal_sum", {
        p_table: table,
        p_column: numberField.key,
      })
      if (!error) {
        metric = {
          label: numberField.label,
          value: Number(data ?? 0),
          currency: numberField.type === "currency",
        }
      }
    }

    // New records over time — bucketed by day on created_at.
    let series: Array<{ bucket: string; value: number }> = []
    {
      const { data, error } = await supabase.rpc("portal_time_series", {
        p_table: table,
        p_column: "created_at",
        p_bucket: "day",
      })
      if (!error && Array.isArray(data)) {
        series = data.map((r: { bucket: unknown; value: unknown }) => ({
    ...
    const payload = {
      moduleId: module?.id ?? table,
      total,
      metric,
      groups,
      series,
      recent: camelizeDeep(recentRows ?? []),
    }
    return ok(payload as unknown as T)
  }
